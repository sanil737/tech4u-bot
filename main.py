...
async def on_message(message):
    ...
    # 3. EXACT VOUCH MONITOR
    if message.channel.id == VOUCH_CHANNEL_ID:
        uid = str(message.author.id)
        user_v_data = vouch_col.find_one({"_id": uid})
        
        if user_v_data and user_v_data.get("permits", 0) > 0:
            # Check if message matches ANY of their expected vouch strings
            if message.content in user_v_data.get("expected_msgs", []):
                vouch_col.update_one({"_id": uid}, {
                    "$inc": {"permits": -1},
                    "$pull": {"expected_msgs": message.content}
                })
                await message.add_reaction("✅")
                ...
                # WRONG MESSAGE - Delete and send instruction
                await message.delete()
                await message.channel.send(f"❌ {message.author.mention}, you must send the **EXACT** message given by the bot in your private channel!", delete_after=10)
        else:
...
async def redeem(interaction: discord.Interaction, code: str):
    ...
    # Define the EXACT string the bot expects
    vouch_msg = f"{code} I got {item['service']}, thanks @admin"
    await temp.send(f"📢 **VOUCH REQUIRED IN <#{VOUCH_CHANNEL_ID}>**:\nPlease **COPY AND PASTE** this exact message:\n`{vouch_msg}`\n*Failure = 3 Day Ban!*")
    
    # Save the expected string to Database
    vouch_col.update_one({"_id": str(member.id)}, {
        "$inc": {"permits": 1},
        "$push": {"expected_msgs": vouch_msg}
    }, upsert=True)
