...
class EGBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        # Auto-Create Helper Role
        for guild in self.guilds:
            role = discord.utils.get(guild.roles, name=HELPER_ROLE_NAME)
            if not role:
                try:
                    await guild.create_role(name=HELPER_ROLE_NAME, color=discord.Color.gold(), hover=True, reason="EG Bot Helper Role")
                    print(f"✅ Created role '{HELPER_ROLE_NAME}' in {guild.name}")
...
@bot.tree.command(name="make", description="Admin: Assign Helper Role")
@app_commands.describe(role="Role type (Helper)")
...
async def winner(interaction: discord.Interaction, winner: discord.Member, score: str):
    ...
    # Public Log (YOUR FORMAT)
    res_chan = bot.get_channel(CH_MATCH_RESULTS)
    if res_chan:
        embed = discord.Embed(title="🏆 MATCH RESULT SUBMITTED", color=discord.Color.green())
        embed.add_field(name="🎮 Game", value="Free Fire", inline=False)
        embed.add_field(name="🆔 Room ID", value=game_id, inline=True)
        embed.add_field(name="🥇 Winner", value=winner.mention, inline=True)
        embed.add_field(name="📊 Score", value=score, inline=False)
        embed.add_field(name="✍️ Result by", value=f"⭐ {interaction.user.mention}", inline=True)
        if helper_msg: embed.add_field(name="💰 Helper Reward", value="+10 Coins", inline=True)
...
class AcceptMatchView(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        overwrites = {guild.default_role: discord.PermissionOverwrite(read_messages=False), challenger: discord.PermissionOverwrite(read_messages=True, send_messages=True), opponent: discord.PermissionOverwrite(read_messages=True, send_messages=True), guild.me: discord.PermissionOverwrite(read_messages=True)}
...
async def boostshop(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="🎮 Boost Shop", description=f"**Coins:** 💰 {d['coins']}\n\n🛒 **Available:**", color=discord.Color.gold())
    text = ""
    for k, v in BOOSTS.items(): text += f"**{v['name']}** • `💰 {v['price']}`\n{v['desc']}\n\n"
    embed.add_field(name="Boosts", value=text)
...
@bot.tree.command(name="adduser", description="Add user to room (100 coins)")
...
@bot.tree.command(name="addtime", description="Extend time (100/hr)")
...
@bot.tree.command(name="prices", description="Show prices")
