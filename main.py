...
app = Flask('')
@app.route('/')
def home(): return "Tech4U Master Bot Active 24/7!"
def run_flask():
...
REDEEM_LOG_CHANNEL_ID = 1457623750475387136 # <--- New Usage Log Channel
...
async def on_message(message):
    ...
    if any(message.content.lower().startswith(c) for c in owo_cmds):
        if message.channel.id != OWO_CHANNEL_ID and not message.author.guild_permissions.administrator:
            ...
            return await message.channel.send(f"🚨 {message.author.mention} Use OwO commands in <#{OWO_CHANNEL_ID}>!", delete_after=5)
...
async def redeem(interaction: discord.Interaction, code: str):
    ...
    # SEND USAGE LOG MESSAGE
    usage_chan = bot.get_channel(REDEEM_LOG_CHANNEL_ID)
    if usage_chan:
        await usage_chan.send(f"**[{code}]** have been use by {member.mention}")
    ...
    if is_yt:
        await temp.send(f"{member.mention} ⚠️ **YouTube Premium**\nPlease type your Gmail address here.")
        try:
